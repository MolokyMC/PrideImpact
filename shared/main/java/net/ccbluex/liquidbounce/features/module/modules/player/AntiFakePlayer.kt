...
import net.ccbluex.liquidbounce.features.module.modules.settings.ClientSettings
...
class AntiFakePlayer : Module() {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is SPacketChat && packet.chatComponent.unformattedText.contains(
            ...
            if (hideAntiGetNameValue.get()) {
                event.cancelEvent()
                if (chatHiddenValue.get()) displayChatMessage("§b$CLIENT_NAME §7» §c隐藏了AntiGetname消息。")
            }
        }
        ...
        if (packet is SPacketChat) {
            ...
            if (modeValue.get().toLowerCase() == "kitbattle" && chat.startsWith("花雨庭")) {
                // 职业隐藏银币消息
                if (kitHideKillCoinValue.get() && ((chat.contains("花雨庭 >>你消灭") && chat.contains("% 的伤害并且获得了") && chat.contains(
                        "硬币!"
    ...
    private fun playerDeathAction(name: String, cd: Long) {
        try {
            bots++
            if (fileManager.friendsConfig.isFriend(name)) fileManager.friendsConfig.removeFriend(name)
        ...
        when (mode.toLowerCase()) {
            "add" -> {
                when (logStyleValue.get().toLowerCase()) {
                    "ryfnew" -> displayChatMessage("${ClientSettings.ccolor}${ClientSettings.cname} §7${ClientSettings.prefix} §aAdded§f HYT Bot §7-> §e$name")
                    "fdpantibot" -> displayChatMessage("§7[§cAntiBot§7] §fAdded §7$name§f due to it being a bot.")
                    "fdpchat" -> displayChatMessage("§f[§c!§f] ${ClientSettings.ccolor}${ClientSettings.cname} §7>> §aAdded §6HYT bot§f[$name§f]§6.")
                    "leave" -> displayChatMessage("${ClientSettings.ccolor}${ClientSettings.cname} §8[§eWARNING§8] §6添加无敌人: $name")
                    "special" -> displayChatMessage("§8[§d${ClientSettings.cname}§8] §a$name§d被§bRyF§d吃掉啦! §bCiallo(∠・ω< )⌒☆")
                    "wawa1" -> displayChatMessage("§6${ClientSettings.cname} §7=> §fAdded Bot §7$name§f.")
                    "wawa2" -> displayChatMessage("§6${ClientSettings.cname} §7${ClientSettings.prefix} §f玩家死亡: §7$name")
                    "nullclient" -> displayChatMessage("§7[§cAntiBots§7] §fAdded a bot(§7$name§f)")
                    "normal" -> displayChatMessage("§7[§6${ClientSettings.cname}§7] §fAdded HYT Bot: §7$name")
                    "windx" -> displayChatMessage("§7[§c!§7] ${ClientSettings.ccolor}${ClientSettings.cname} §aClient §7=> §aAdded §fa bot(§7$name§f)")
                    "old" -> displayChatMessage("§8[§c§l${ClientSettings.cname}§8] §d添加无敌人：§7$name")
                    "arene" -> displayChatMessage("§7[§f${ClientSettings.cname}§7] §fAdd a Bot(§7$name§f)")
                }
            }

            "remove" -> {
                when (logStyleValue.get().toLowerCase()) {
                    "ryfnew" -> displayChatMessage("${ClientSettings.ccolor}${ClientSettings.cname} §7${ClientSettings.prefix} §cRemoved§f HYT Bot §7-> §e$name")
                    "fdpantibot" -> displayChatMessage("§7[§cAntiBot§7] §fRemoved §7$name§f due to respawn.")
                    "fdpchat" -> displayChatMessage("§f[§c!§f] ${ClientSettings.ccolor}${ClientSettings.cname} §7>> §cRemoved §6HYT bot§f[$name§f]§6.")
                    "leave" -> displayChatMessage("${ClientSettings.ccolor}${ClientSettings.cname} §8[§eWARNING§8] §6删除无敌人: $name")
                    "special" -> displayChatMessage("§8[§d${ClientSettings.cname}§8] §a$name§d被§bRyF§d吐出来咯~ §bCiallo(∠・ω< )⌒☆")
                    "wawa1" -> displayChatMessage("§6${ClientSettings.cname} §7=> §fRemoved Bot §7$name§f.")
                    "wawa2" -> displayChatMessage("§6${ClientSettings.cname} §7${ClientSettings.prefix} §f玩家重生: §7$name")
                    "nullclient" -> displayChatMessage("§7[§cAntiBots§7] §fRemoved a bot(§7$name§f)")
                    "normal" -> displayChatMessage("§7[§6${ClientSettings.cname}§7] §fRemoved HYT Bot: §7$name")
                    "windx" -> displayChatMessage("§7[§c!§7] ${ClientSettings.ccolor}${ClientSettings.cname} §aClient §7=> §cRemoved §fa bot(§7$name§f)")
                    "old" -> displayChatMessage("§8[§c§l${ClientSettings.cname}§8] §d删除无敌人：§7$name")
                    "arene" -> displayChatMessage("§7[§f${ClientSettings.cname}§7] §fDel a Bot(§7$name§f)")
                }
            }
        }
    }
