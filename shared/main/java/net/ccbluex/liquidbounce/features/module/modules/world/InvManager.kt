...
class InvManager : Module() {
    ...
    private val ignoreChainArmorValue = BoolValue("NoChainArmor", false)
    private val ignoreVehiclesValue = BoolValue("IgnoreVehicles", false)
    private val randomSlotValue = BoolValue("SlotRandom", false)
    ...
    fun isUseful(itemStack: IItemStack, slot: Int): Boolean {
        return try {
            ...
            } else (classProvider.isItemFood(item) && classProvider.isItemAppleGold(item)) || itemStack.unlocalizedName == "item.arrow" || itemStack.unlocalizedName == "item.slimeball" || (classProvider.isItemBlock(item) && !InventoryUtils.BLOCK_BLACKLIST.contains(item!!.asItemBlock().block)) || classProvider.isItemBed(item) || classProvider.isItemPotion(
                item
    ...
    private fun move(item: Int, isArmorSlot: Boolean): Boolean {
        if (!isArmorSlot && item < 9 && !classProvider.isGuiInventory(mc.currentScreen)) {
            ...
            mc.netHandler.addToSendQueue(
                createUseItemPacket(
                    mc.thePlayer!!.inventory.getCurrentItemInHand(),WEnumHand.MAIN_HAND
                )
            )
            mc.netHandler.addToSendQueue(
