...
//        mc.getThePlayer().setMotionZ(xzMultiplier.get());
        if (mc.getThePlayer().getOnGround()){
            ...
            rotations = rotationsValue.get();
        }

        // Lock Rotation
        if (rotations && keepRotationValue.get() && lockRotation != null)
...
//        }
        ...
        if ((!rotations || noHitCheckValue.get() || faceBlock) && placeModeValue.get().equalsIgnoreCase(eventState.getStateName()) && !towerActivation()) {
            place(false);
        }
        ...
        if (eventState == EventState.PRE) {
            ...
            if (InventoryUtils.findAutoBlockBlock() != -1 || isHeldItemBlock) {
                ...
                if (mc.getTheWorld().getBlockState(WBlockPos).getBlock() instanceof BlockAir) {
                    if (search(WBlockPos, true, true) && rotations) {
