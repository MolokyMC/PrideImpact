...
class SuperKnockback : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf("Normal", "MCYC", "ExtraPacket", "PrideImpact", "Packet", "HYTPacket", "Tick" ), "Normal")
    val onlyMoveValue = BoolValue("OnlyMove", false)
    ...
    fun onAttack(event: AttackEvent) {
        if (classProvider.isEntityLivingBase(event.targetEntity)) {
            val target = event.targetEntity!!.asEntityLivingBase()
            if (target.hurtTime > hurtTimeValue.get() || !timer.hasTimePassed(delay.get().toLong()) ||
