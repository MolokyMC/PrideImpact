...
@ModuleInfo(name = "KAFix", description = "fix ur ka", category = ModuleCategory.COMBAT)
class KAFix : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        if (mc.thePlayer!!.isAirBorne){
            ...
            if(AirRange.get() != killAura.rangeValue.get()) killAura.rangeValue.set(AirRange.get())
            if(Debug.get()){
        ...
        if (mc.thePlayer!!.onGround){
            val killAura = LiquidBounce.moduleManager[KillAura::class.java] as KillAura
            if(GroundRange.get() != killAura.rangeValue.get()) killAura.rangeValue.set(GroundRange.get())
            if(Debug.get()){
        ...
        if (ticks ==1){
            val killAura = LiquidBounce.moduleManager[KillAura::class.java] as KillAura
            if(hurttime.get() != killAura.hurtTimeValue.get()) killAura.hurtTimeValue.set(hurttime.get())
            if(Debug.get()){
        ...
        if (ticks ==2){
            val killAura = LiquidBounce.moduleManager[KillAura::class.java] as KillAura
            if(hurttime2.get() != killAura.hurtTimeValue.get()) killAura.hurtTimeValue.set(hurttime2.get())
            if(Debug.get()){
