...
@ModuleInfo(name = "HytGetName", description = "Fixed By WaWa", category =ModuleCategory.MISC)
class HytGetName : Module() {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (classProvider.isSPacketChat(packet)) {
            ...
            // Anti AntiGetName
            if (packet.asSPacketChat().chatComponent.unformattedText.contains(":")) return
                ...
                "4v4/1v1", "32/64" -> {
                    ...
                    if (matcher.find()) {
                        ...
                        if (name != "") {
                            ...
                            ClientUtils.displayChatMessage("§8[§c§lPrideImpact提醒您§8]§c§d添加无敌人：" + name)
                            Thread {
                                try {
                                    Thread.sleep(5000)
                                    LiquidBounce.fileManager.friendsConfig.removeFriend(name)
                                    ClientUtils.displayChatMessage("§8[§c§lPrideImpact提醒您§8]§c§d删除无敌人：" + name)
