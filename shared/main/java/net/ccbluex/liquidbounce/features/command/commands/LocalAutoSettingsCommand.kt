...
class LocalAutoSettingsCommand : Command("localautosettings", "localsetting", "localsettings", "localconfig") {
    ...
    override fun execute(args: Array<String>) {
        if (args.size > 1) {
            when {
                args[1].equals("download",ignoreCase = true) -> {
                    if (args.size > 2){
                        ...
                        try {
                            ...
                            chat("§c下载中,等待两秒后请输入.localautosettings load 读取参数")
                            HttpUtils.download("https://gitee.com/set-dow/light-sense/tree/master/$option",LiquidBounce.fileManager.settingsDir)
                    ...
                    if (args.size > 2) {
                        ...
                        if (scriptFile.exists()) {
                            try {
                                ...
                                LiquidBounce.hud.addNotification(Notification("Notification","Updated Settings", NotifyType.INFO))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
        ...
        chatSyntax("localsettings <load/save/list/delete/download>")
    }
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        return when (args.size) {
            1 -> listOf("delete", "list", "load", "save","download").filter { it.startsWith(args[0], true) }
            2 -> {
