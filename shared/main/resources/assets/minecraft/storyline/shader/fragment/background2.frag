...
void main( void ) {
    ...
    //"folded" texture
    vec2 q = vec2(0.);
    q.x = fbm(gl_FragCoord.xy/iResolution.xy+iTime*0.01);
    q.y = fbm(gl_FragCoord.xy/iResolution.xy+iTime*0.01);
    // preview q
    color += vec3(q.x, 0., 0.);
    ...
    color = mix(color,
		...
		//vec3(0.5 + 0.5*sin(iTime+st.yxx+vec3(0,2,4))),
                1.0);
    color = mix(vec3(0.9,0.1,0.4),
                vec3(0.8029, 0.8029, 0.8055),
		//vec3(0.5 + 0.5*sin(iTime+st.yyx+vec3(0,2,4))),
